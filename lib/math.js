 ...
 * @type {Function} getResultPod
 * @param {Array} pods Wolfram pods
 * @return {Promise} promise
 */
var getResultPod = (pods) => {
  var dfd = q.defer()
  ...
  var funcArray = [
    ( callback ) => { // Sweep pods looking for result id
      ...
      _.forEach(pods, (value) => {
        if(((value || {}).$ || {}).id === 'Result') return callback(null, value)
        count++
        if (count == pods.length) callback({message: 'No result found!'})
      })
    }
  ]
